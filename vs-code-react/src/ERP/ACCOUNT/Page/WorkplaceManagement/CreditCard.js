...
const CreditCard = () => {
    ...
    const CreditCardGrid = [
        // 칼럼정의
        { checkboxSelection: true, rowSelection: 'multiple', rowMultiSelectWithClick: true, width: "80" },
        ...
        { headerName: "가맹점 번호", field: "cardOpenPlace", editable: true, },
        { headerName: "카드번호", field: "cardNumber", editable: true, },
    ...
    const deleteBtn = () => {
        ...
        if (selectedData.length === 0) {   // 선택된게 없으면.
            ...
            return;
        } if (!window.confirm("⛔ " + selectedData[0].cardMemberName + "님 신용카드 정보를 삭제 하시겠습니까 ? ⛔ ")) {
            alert("🧡💛💚💙💜🤎🖤🤍");
            return;
        } else {
            dispatch({
    ...
    const saveBtn = () => {
        ...
        if (!window.confirm(" 저장하시겠습니까 ? ")) {
            ...
            for (var i = 0; i < rowsCount; i++) {
                ...
                delete rows[i].chk
                //|| rows[i].customerName === null || rows[i].cardOpenPlace === null || rows[i].cardNumber === null || rows[i].customerType === null
                if (rows[i].cardMemberName === null) {
                    alert("빈칸없이 입력해주세요.");
                    return;
                }
    ...
    return <>
        ...
        >
            <AgGridReact
                columnDefs={CreditCardGrid} //정의된 컬럼 
