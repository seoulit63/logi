...
const NormalAccount = () => {
    ...
    const NormalAccountGrid = [
        ...
        { headerName: "거래처 코드", field: "customerCode", sortable: true, width: "240", }, // sortable 컬럼눌러서 오름차순내림차순 가능 
    ...
    const deleteBtn = () => {
        ...
        if (selectedData.length === 0) {   // 선택된게 없으면.
            ...
            return;
        } if (!window.confirm(selectedData[0].customerName + " 거래처 정보를 삭제 하시겠습니까 ? ")) {
            alert(" 취소되었습니다. ");
            return;
        } else {
            dispatch({
